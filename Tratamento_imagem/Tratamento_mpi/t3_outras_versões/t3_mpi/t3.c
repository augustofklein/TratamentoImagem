...
void preenche_matriz(RGB **m, int nproc, int id, char entrada[100]){
...
void calcula_mediana(RGB ** m, int nproc, int id, int mascara){
...
int main(int argc, char **argv){
	...
	vet_imagem = aloca_matriz(largura, altura);
	...
	for(i=0;i<nproc;i++){
		if (id == i){
			...
			if (i == 0){
				preenche_matriz(vet_imagem, nproc, id, "b2.bmp");
				// Caso houver mais de um processo
				if (nproc > 1){
					MPI_Send(&vet_imagem, sizeof(RGB) * (largura*altura), MPI_UNSIGNED_CHAR, i+1, 100, MPI_COMM_WORLD);
				}
			}else{
				MPI_Recv(&vet_imagem, sizeof(RGB) * (largura*altura), MPI_UNSIGNED_CHAR, i-1, 100, MPI_COMM_WORLD, &s);
				preenche_matriz(vet_imagem, nproc, id, "b2.bmp");
				if (nproc-1 != i){
					MPI_Send(&vet_imagem, sizeof(RGB) * (largura*altura), MPI_UNSIGNED_CHAR, i+1, 100, MPI_COMM_WORLD);
				}
	...
	for(i=0;i<nproc;i++){
		if (id == i){
			// Primeiro elemento apenas envia e não recebe
			if (i == 0){
				calcula_mediana(vet_imagem, nproc, id, tam_mascara);
				// Caso houver mais de um processo
				if (nproc > 1){
					MPI_Send(&vet_imagem, sizeof(RGB) * (largura*altura), MPI_UNSIGNED_CHAR, i+1, 100, MPI_COMM_WORLD);
				}
	...
	if(id == 0){
		...
		for(i=0;i<cabecalho.altura;i++){
			...
			for(j=0;j<cabecalho.largura;j++){
				fwrite(&vet_imagem[i][j], sizeof(RGB), 1, fout);
			}
